...
class MbMetersim(MbSlave):
    ...
    def __init__(self, slave_id, sem):
        super().__init__(slave_id)
        self.sem = sem
...
class ModbusSimulator:
    ...
    def __init__(
        ...
        # Metersim
        mbSem = MbMetersim(ADDRESS_METERSIN, sem)
        ...
        mbSimControl.set_offload_freq(3600)
        mbSimControl.set_training_freq(3600 * 24)
        self.devices.append(mbSimControl)
    ...
    async def start_serial(self, serial_dev: Path):
        task_updating = asyncio.create_task(self.updating_task())
        await StartAsyncSerialServer(
            context=self.context,
            port=serial_dev.as_posix(),
            stopbits=1,
            bytesize=8,
            parity="N",
            baudrate=9600,
    ...
    def run_serial(self, serial_dev: Path):
        asyncio.run(self.start_serial(serial_dev))
