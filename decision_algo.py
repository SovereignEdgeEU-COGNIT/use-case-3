def run_one_step(
    ...
    Args:
        ...
        energy_drawn_from_grid: kWh
        energy_returned_to_grid: kWh
        energy_pv_produced: kWh
        temp_outdoor: °C
    ...
    def check_heating_conditions(
        ...
        for room_heating_params in rooms_list:
            ...
            temp_diff = (
                room_heating_params["preferred_temp"] - reduction_of_allowed_temp - room_temp
            )
            if temp_diff > 0:
                energy_for_room_heating = (
                    sum(room_heating_params["powers_of_heating_devices"]) * step_timedelta_s / 3600
                )  # kWh
                if available_energy:
    ...
    def distribute_needed_energy(
        energy_needed: float, energy_pv_produced: float, energy_in_storage: float
    ...
    energy_in_storage = (
        (charge_level_of_storage - storage_min_charge_level) / 100 * storage_max_capacity
    )
    ...
    for room_heating_params in room_heating_params_list:
        total_power_used_for_heating += sum(
            ...
            for power, status in zip(
                room_heating_params["powers_of_heating_devices"],
                heating_status_per_room[room_heating_params["name"]],
            )
        )
    energy_consumption_without_heating = max(
        energy_consumption - total_power_used_for_heating * step_timedelta_s / 3600, 0.0
    )
    ...
    (energy_for_necessary_heating, conf_per_room, temp_per_room_pred, remaining_rooms_list) = (
    ...
    available_oze_energy = energy_pv_produced + max(
        energy_in_storage
        - model_parameters["storage_high_charge_level"] / 100 * storage_max_capacity,
        0.0,
    )
    if available_oze_energy > 0.0 and len(remaining_rooms_list) > 0:
    ...
    for room in remaining_rooms_list:
        ...
        next_temp_per_room[room_name] = (
            room_temp
            - (room_temp - temp_outdoor) * heat_loss_coeff * step_timedelta_s / heat_capacity
        )
    ...
    if energy_in_storage > initial_energy_in_storage:
        ...
        energy_to_charge_storage = (
            energy_in_storage - initial_energy_in_storage
        ) / storage_efficiency
        energy_storage_configuration["InWRte"] = min(
            round(
                energy_to_charge_storage / (step_timedelta_s / 3600) / storage_nominal_power * 100.0
                + model_parameters["storage_delta_power_perc"],
                2,
            ),
            100.0,
        )
        ...
        energy_storage_configuration["OutWRte"] = min(
            round(
                energy_to_discharge_storage
                / (step_timedelta_s / 3600)
                / storage_nominal_power
                * 100.0
                + model_parameters["storage_delta_power_perc"],
                2,
            ),
