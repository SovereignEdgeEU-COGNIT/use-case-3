...
from device_simulators.device_utils import complex_dot_product, DeviceUserApi
from device_simulators.simple_device import ScheduledDevice
...
class RoomHeating(Device, DeviceUserApi):
    ...
    def updateTemp(self, now: int):
        ...
        denergy = (
            ...
            * sum(
                is_switch_on * power
                for is_switch_on, power in zip(self.is_device_switch_on, self.heating_devices_power)
            )
            * 1000
            * dt
    ...
    def update(self, info: InfoForDevice) -> DeviceResponse:
        ...
        if info.voltage[0] != 0:
            self.current[0] = (
                sum(
                    is_switch_on * power
                    for is_switch_on, power in zip(
                        self.is_device_switch_on, self.heating_devices_power
                    )
                )
                * 1000
                / info.voltage[0]
